...
PRETRAINED_MODEL_ARCHIVE_MAP = {
    ...
    'bert-large-uncased-whole-word-masking': "/home/Facundo/Downloads/pytorch-pretrained-BERT/bert-large-uncased-whole-word-masking-pytorch_model.bin",
...
PRETRAINED_CONFIG_ARCHIVE_MAP = {
    ...
    'bert-large-uncased-whole-word-masking': "/home/Facundo/Downloads/pytorch-pretrained-BERT/bert-large-uncased-whole-word-masking-config.json",
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        #self.word_embeddings_dropout = nn.Dropout(p=0.3)
    ...
    def forward(self, input_ids, token_type_ids=None):
        ...
        #word_embeddings_dropout = self.word_embeddings_dropout(words_embeddings)

        #print("----size of words_embeddings--------")
        #print(words_embeddings.size())
        position_embeddings = self.position_embeddings(position_ids)
        token_type_embeddings = self.token_type_embeddings(token_type_ids)

        #embeddings = word_embeddings_dropout + position_embeddings + token_type_embeddings
...
class BertEmbeddings2(nn.Module):
    ...
    """
    def __init__(self, config):
        super(BertEmbeddings2, self).__init__()
        self.word_embeddings = nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=0)
        self.position_embeddings = nn.Embedding(config.max_position_embeddings, config.hidden_size)
        self.token_type_embeddings = nn.Embedding(config.type_vocab_size, config.hidden_size)

        #self.word_embeddings_dropout = nn.Dropout(p=0.5)
    ...
    def forward(self, input_ids, token_type_ids=None, drop_wordembedding=-1):
        seq_length = input_ids.size(1)
        position_ids = torch.arange(seq_length, dtype=torch.long, device=input_ids.device)
        position_ids = position_ids.unsqueeze(0).expand_as(input_ids)
        if token_type_ids is None:
        ...
        #print("-------------dropout----------------------")
        #print(drop_wordembedding)
        #print(words_embeddings.size())

        #print(words_embeddings[0][drop_wordembedding])
        word_embeddings_dropout = nn.functional.dropout(words_embeddings[0][drop_wordembedding],p=1,training=True)
        words_embeddings[0][drop_wordembedding] = word_embeddings_dropout
        #print(words_embeddings[0][drop_wordembedding])
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config, output_attentions=False, keep_multihead_output=False):
        ...
        self.embeddings = BertEmbeddings2(config)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, drop_wordembedding = -1, output_all_encoded_layers=True, head_mask=None):
        ...
        embedding_output = self.embeddings(input_ids, token_type_ids, drop_wordembedding)
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, drop_wordembedding=-1, masked_lm_labels=None, head_mask=None):
        outputs = self.bert(input_ids, token_type_ids, attention_mask, drop_wordembedding, 
        ...
        if masked_lm_labels is not None:
            loss_fct = CrossEntropyLoss(ignore_index=-1)
            #print("size")
            #print(prediction_scores.view(-1, self.config.vocab_size).size())
            masked_lm_loss = loss_fct(prediction_scores.view(-1, self.config.vocab_size), masked_lm_labels.view(-1))
